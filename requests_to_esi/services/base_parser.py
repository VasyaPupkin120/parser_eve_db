...
async def create_all_entities(action:action_list_type, entity:entity_list_type):
    ...
    Если нужно не внести данные по целой модели и дополнить поле модели, эта функция
    ...
    все поля, а обновить только одно. Внутренние id для этой цели можно заранее
    выставлять пустыми - [].
    """
